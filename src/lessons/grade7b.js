...
export const grade7BLessons=grade7BCourses.flatMap(course=>{
 ...
 return course.lessons.map((entry,index)=>{
  ...
  if(entry.examCode)return {...common,summary:false,examId:'7B-'+(geography?'GEO':'GP')+'-'+entry.examCode,stages:[
   s('prepare','Prepare',5,0,notes+'\nUse the '+(entry.examCode==='A0'?20:12)+'-question paper. Read instructions aloud if helpful, without explaining answers. Baseline identifies starting points; it is not expected mastery.',[f(subject+' assessment','listen',[(entry.examCode==='A0'?20:12)+' questions. Four choices each.','Circle one answer for each question.'],{printExam:true}),f('Ready to begin','listen',['Write your name, class and date.','Work independently. Ask if an instruction is unclear.'])]),
   ...
   s('reflect','Collect and reflect',7,30,'Collect papers before opening the separate key in Teacher tools. Keep practical teamwork and speaking evidence alongside the MCQ result.',[f('Check and hand in','think',['Check your name and all '+(entry.examCode==='A0'?20:12)+' responses.','Choose one skill you want to practise.'])]),
