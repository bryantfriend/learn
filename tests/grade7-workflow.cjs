...
(async()=>{
 ...
 await page.locator('.gp-diagram').evaluate(async img=>{await img.decode();if(!img.naturalWidth)throw Error('Diagram failed to load');});await page.screenshot({path:'output/playwright/g7-grid-board.png'});
 ...
 assert.equal(await page.locator('[data-action="g7-plan"]').count(),0);
