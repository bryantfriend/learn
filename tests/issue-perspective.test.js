...
test('rewritten lesson discards outdated answers while preserving class and preferences',()=>{
 ...
 assert.ok(migrated);assert.equal(migrated.contentRevision,2);assert.equal(migrated.stage,0);
 ...
 const current={...migrated,stage:3,steps:[0,0,0,2,0],responses:{'3:2':{selected:'A',revealed:true}}};
 assert.deepEqual(validateSession(current,2000),current);
 assert.equal(validateSession({...current,contentRevision:3},2000),null);
});
