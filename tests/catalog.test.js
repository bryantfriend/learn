...
test('all five class/subject combinations expose starters; future lessons filter by grade and subject',()=>{
 ...
 for(const c of classes)for(const s of subjectsFor(c.id))assert.equal(lessonsFor(c.id,s.id).length,c.id==='8'?45:c.id==='7a'?(s.id==='geography'?35:64):(s.id==='geography'?65:95));
