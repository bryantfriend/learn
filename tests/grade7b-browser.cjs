...
(async()=>{
 const {grade7BLessons}=await import('../src/lessons/grade7b.js');
 await new Promise(r=>server.listen(4181,'127.0.0.1',r));
 browser=await chromium.launch({headless:true});
 const url=process.env.LEARN_URL||'http://127.0.0.1:4181/learn/';
 fs.mkdirSync('output/playwright',{recursive:true});
 const errors=[];
 for(const lesson of (process.env.PRINT_ONLY?[]:grade7BLessons)){
  const context=await browser.newContext({viewport:{width:1280,height:720}});
  await context.addInitScript(l=>{
   const key='learn.session.v1';
   if(!localStorage.getItem(key))localStorage.setItem(key,JSON.stringify({schemaVersion:3,lessonId:l.id,classId:'7b',subjectId:l.catalog.subjectId,classLabel:'7B',stage:0,steps:l.stages.map(()=>0),responses:{},stars:0,preferences:{starsVisible:false,timerVisible:true},timer:{durationMs:60000,remainingMs:60000,running:false,deadline:null},seenFrames:{'0:0':true},attentionReturns:{},discussedQuestions:[],startedAt:Date.now(),finishedAt:null}));
  },lesson);
  ...
  if(lesson.id==='g7b-geo-w02-1'){
   ...
   await page.screenshot({path:'output/playwright/g7b-lesson-complete.png'});
  }
 ...
 for(const id of Object.keys((await import('../src/lessons/g7b-exams.js')).grade7BExams))for(const key of [false,true]){
  await printPage.goto(url+'exams/?id='+id+(key?'&key=1':''));
  await printPage.waitForSelector('.paper-sheet');
  const count=id.endsWith('A0')?20:12;
  ...
  if(id==='7B-GEO-Q1')await printPage.locator('.paper-sheet').first().screenshot({path:'output/playwright/g7b-exam-q1-'+(key?'key':'student')+'.png'});
