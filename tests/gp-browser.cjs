...
(async()=>{
 const {gpLessons}=await import('../src/lessons/grade8-gp.js');
 await new Promise(r=>server.listen(4177,'127.0.0.1',r));
 browser=await chromium.launch({headless:true});
 const url=process.env.LEARN_URL||'http://127.0.0.1:4177/learn/';
 fs.mkdirSync('output/playwright',{recursive:true});
 const errors=[];
 for(const lesson of gpLessons){
  const context=await browser.newContext({viewport:{width:1280,height:720}});
  await context.addInitScript(l=>{
   const key='learn.session.v1';
   if(!localStorage.getItem(key))localStorage.setItem(key,JSON.stringify({schemaVersion:3,lessonId:l.id,classId:'8',subjectId:'global-perspectives',classLabel:'8th Grade',stage:0,steps:l.stages.map(()=>0),responses:{},stars:0,preferences:{starsVisible:false,timerVisible:true},timer:{durationMs:60000,remainingMs:60000,running:false,deadline:null},seenFrames:{'0:0':true},attentionReturns:{},discussedQuestions:[],startedAt:Date.now(),finishedAt:null}));
  },lesson);
  ...
  async function fit(label){
   const bad=await page.evaluate(()=>{
    ...
    for(const node of document.querySelectorAll('.copy h1,.instructions,.answers,.explanation,.footnote,.exam-link')){
  ...
  if(lesson.id==='g8-gp-1.1'){
   await page.locator('[data-action="summary"]').click();assert.match(await page.locator('[data-summary="stages"]').innerText(),/6 \/ 6/);
   ...
   await page.screenshot({path:'output/playwright/gp-lesson-complete.png'});
  }
 ...
 for(const id of ['A0','Q1','Q2','Q3','Q4'])for(const key of [false,true]){
  ...
  if(id==='Q1')await printPage.locator('.paper-sheet').first().screenshot({path:'output/playwright/exam-q1-'+(key?'key':'student')+'.png'});
