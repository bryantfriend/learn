...
(async()=>{
 await new Promise(r=>server.listen(4182,'127.0.0.1',r));browser=await chromium.launch({headless:true});
 ...
 await page.goto(process.env.LEARN_URL||'http://127.0.0.1:4182/learn/');
 ...
 await a('choose-lesson');await page.locator('[data-id="7b"]').click();await page.locator('[data-id="geography"]').click();await a('g7-plan');
 ...
 await page.locator('[data-quarter="Q1"]').click();await page.screenshot({path:'output/playwright/g7b-weekly-plan.png'});
 ...
 assert.match(await page.locator('#panel').innerText(),/3 lesson\(s\) per teaching week/);
 await a('back-picker');await page.locator('[data-id="7a"]').click();
 assert.equal(await page.locator('[data-lesson^="g7b-"]').count(),0);
 assert.ok(await page.locator('[data-action="select-lesson"]').count()>2);
 assert.deepEqual(errors,[]);console.log('PASS 7B weekly plan, responsive layout, diagram, exam, print, key and 7A isolation');
